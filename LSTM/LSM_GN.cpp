...
// 三次B样条插值系数表
float**** bicubic_Bspline(cv::Mat image);
// 创建四维矩阵指针
float**** new4D(int dimension1, int dimension2, int dimension3, int dimension4);
// 释放四维矩阵指针
void delete4D(float****& ptr);
// 根据插值表计算插值像素强度
float bicubic_Bspline_calculation(int I, int J, float x2, float y2, float**** lookup_table);
// 根据插值表计算x梯度
float gradientX(int I, int J, float x2, float y2, float**** lookup_table);
// 根据插值表计算y梯度
float gradientY(int I, int J, float x2, float y2, float**** lookup_table);
...
int main(int argc, char ** argv)
{
	...
	cv::Mat H;
	double reprojectionThreshold = 0.01;
	...
	clock_t finish_t2 = clock();
	long duration2 = (finish_t2 - start_t2) / CLOCKS_PER_SEC;
	std::cout << "Total time: " << duration2 << " seconds." << std::endl;
	cin.get();
...
float Get_coefficient(cv::Mat matchLeftWindow, cv::Mat imageRight, int x, int y)
{
	...
	for (int m = 0; m < matchLeftWindow.rows; m++)
	{
		for (int n = 0; n < matchLeftWindow.cols; n++)
		{
			cofficent4 += pow((matchLeftWindow.at<float>(m, n) / sqrt(cofficent2)) - (Rmatchwindow.at<float>(m, n) / sqrt(cofficent3)), 2);
		}
	}
	
	//float cofficent = cofficent1 / sqrt(cofficent2 * cofficent3);
	float cofficent = cofficent4;
	/*
	float sumL = 0;
	float sumR = 0;
	for (int m = 0; m < matchLeftWindow.rows; m++)
...
int LSM_Matching(cv::Mat ImgL, cv::Mat ImgR, std::vector<cv::Point3f> featurePointLeft, std::vector<cv::Point3f> featurePointRight_sift, cv::Mat H, std::vector<cv::Point3f>& featureLeftPointLST, std::vector<cv::Point3f>& featureRightPointLST)
{
	...
	for (int i = 0; i < featurePointLeft.size(); i++)
	{
		...
		if (tempfeatureRightPoint.at(0).z > lowst_door && tempfeatureRightPoint.at(0).z < 1)  // ZNSSD距离约束，未启用
		{
			cv::Point3f tempr;
	...
	//开始进行最小二乘匹配
	//计算图像插值查询表
	float**** lookup_table = bicubic_Bspline(imageRight);
	float**** lookup_tableL = bicubic_Bspline(imageLeft);
#pragma omp parallel for
	for (int i = 0; i < featurePointLeft.size(); i++)
	{
		...
		*/
		...
		float xs = 0, ys = 0;  // 右窗口中心点
		float xt = 0, yt = 0;  // 左窗口中心点
		int iteration = 0;
		...
		do
		{
			...
			//Eigen::MatrixXf C = Eigen::MatrixXf::Zero(matchsize*matchsize, 8);
			//Eigen::MatrixXf L = Eigen::MatrixXf::Zero(matchsize*matchsize, 1);
			//Eigen::MatrixXf P = Eigen::MatrixXf::Identity(matchsize*matchsize, matchsize*matchsize);
			...
			for (int m = x1 - half_matchsize; m <= x1 + half_matchsize; m++)
			{
				for (int n = y1 - half_matchsize; n <= y1 + half_matchsize; n++)
				{
					...
					if (I <= 1 || I >= imageRight.cols - 1 || J <= 1 || J >= imageRight.rows - 1)
					{
						...
						//int coor = ((n - int(y1 - half_matchsize))*(2 * half_matchsize + 1) + m - (x1 - half_matchsize - 1));
						//I = 2; J = 2; P(coor, coor) = 0;
					}
					/*
					// 三次样条插值
					float GrayIntensity = Rifman(x2 - (I - 1), y2 - J + 1) * imageRight.at<uchar>(cv::Point(I - 1, J - 1)) + Rifman(x2 - I, y2 - J + 1) * imageRight.at<uchar>(cv::Point(I, J - 1)) + Rifman(I + 1 - x2, y2 - J + 1)  * imageRight.at<uchar>(cv::Point(I + 1, J - 1)) + Rifman(I + 2 - x2, y2 - J + 1)  * imageRight.at<uchar>(cv::Point(I + 2, J - 1))
					...
					*/
					/*
					//双线性内插重采样
					float GrayIntensity = (J + 1 - y2)*((I + 1 - x2)*imageRight.at<uchar>(cv::Point(I, J)) + (x2 - I)*imageRight.at<uchar>(cv::Point(I + 1, J)))
						+ (y2 - J)*((I + 1 - x2)*imageRight.at<uchar>(cv::Point(I, J + 1)) + (x2 - I)*imageRight.at<uchar>(cv::Point(I + 1, J + 1)));
					*/
					// 
					float GrayIntensity = bicubic_Bspline_calculation(I, J, x2, y2, lookup_table);

					//辐射校正
					float radioGray = h0 + h1*GrayIntensity;//得到相应灰度
					// 右窗口，用于计算相关系数
					...
					*/
					...
					gx = gradientY(I, J, x2, y2, lookup_table);
					gy = gradientX(I, J, x2, y2, lookup_table);
					
					C.at<float>(dimension, 0) = 1;		C.at<float>(dimension, 1) = GrayIntensity;
					...
					//L.at<float>(dimension, 0) = bicubic_Bspline_calculation(floor(m), floor(n), m, n, lookup_tableL) - radioGray;
					...
					/*
					float gyLeft = 0.0f;
					gyLeft -= imageLeft.at<uchar>(cv::Point(m, n + 2)) / 12.f;
					gyLeft += imageLeft.at<uchar>(cv::Point(m, n + 1)) * (2.f / 3.f);
					gyLeft -= imageLeft.at<uchar>(cv::Point(m, n - 1)) * (2.f / 3.f);
					gyLeft += imageLeft.at<uchar>(cv::Point(m, n - 2)) / 12.f;

					float gxLeft = 0.0f;
					gxLeft -= imageLeft.at<uchar>(cv::Point(m + 2, n)) / 12.f;
					gxLeft += imageLeft.at<uchar>(cv::Point(m + 1, n)) * (2.f / 3.f);
					gxLeft -= imageLeft.at<uchar>(cv::Point(m - 1, n)) * (2.f / 3.f);
					gxLeft += imageLeft.at<uchar>(cv::Point(m - 2, n)) / 12.f;
					*/
					float gyLeft = 0.0f;
					float gxLeft = 0.0f;
					gyLeft = gradientY(floor(m), floor(n), m, n, lookup_tableL);
					gxLeft = gradientX(floor(m), floor(n), m, n, lookup_tableL);
			...
			float cofficent1 = 0.0f;
			float cofficent2 = 0.0f;
			float cofficent3 = 0.0f;
			for (int m = 0; m < LWindow.rows; m++)
			{
				for (int n = 0; n < LWindow.cols; n++)
				{
			...
			//计算辐射畸变和几何变形的参数
			//Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> Nb;
			//Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> Ub;
			//Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> parameter;
			//Nb = C.transpose()*P*C;
			//Ub = C.transpose()*P*L;
			//parameter = Nb.ldlt().solve(Ub);
			//parameter = Nb.inverse() * Ub;
			cv::Mat Nb = C.t()*P*C;
			cv::Mat Ub = C.t()*P*L;
			cv::Mat temp(Nb.rows, Nb.cols, CV_32F);
			double a = cv::invert(Nb, temp, DECOMP_LU);  // DECOMP_CHOLESKY  DECOMP_LU  DECOMP_EIG

			//cv::Mat parameter = Nb.inv()*Ub;
			cv::Mat parameter = temp*Ub;
			...
			/*
			float dh0 = parameter(0, 0); float dh1 = parameter(1, 0);
			float da0 = parameter(2, 0); float da1 = parameter(3, 0); float da2 = parameter(4, 0);
			float db0 = parameter(5, 0); float db1 = parameter(6, 0); float db2 = parameter(7, 0);
			*/
			...
			*/
			dp_norm_max = 0.f;
			dp_norm_max += da0 * da0;
			dp_norm_max += da1 * da1 * half_matchsize * half_matchsize;
			dp_norm_max += da2 * da2 * half_matchsize * half_matchsize;
			dp_norm_max += db0 * db0;
			dp_norm_max += db1 * db1 * half_matchsize * half_matchsize;
			dp_norm_max += db2 * db2 * half_matchsize * half_matchsize;
			dp_norm_max = sqrtf(dp_norm_max);
		}while (iteration <= 100 /*增强鲁棒性*/&& (dp_norm_max >= 0.01f && CorrelationCoe > 0.001));
...
float**** bicubic_Bspline(cv::Mat image)
{
	...
	float**** lookup_table = new4D(height, width, 4, 4);
	Eigen::MatrixXf eg_mat = Eigen::MatrixXf::Zero(height, width);
	cv::cv2eigen(Image, eg_mat);
#pragma omp parallel for
	for (int r = 1; r < height - 2; r++) {
		for (int c = 1; c < width - 2; c++) {
			...
			for (int i = 0; i < 4; i++) {
				for (int j = 0; j < 4; j++) {
					matrix_g[i][j] = eg_mat(r - 1 + i, c - 1 + j);
				}
			}
...
float**** new4D(int dimension1, int dimension2, int dimension3, int dimension4) {
	float**** array4D = new float***[dimension1];
	for (int i = 0; i < dimension1; i++)
	{
		array4D[i] = new float**[dimension2];
	}
	for (int i = 0; i < dimension1; i++)
	{
		for (int j = 0; j < dimension2; j++)
		{
			array4D[i][j] = new float*[dimension3];
		}
	}
	for (int i = 0; i < dimension1; i++)
	{
		for (int j = 0; j < dimension2; j++)
		{
			...
			{
				array4D[i][j][k] = new float[dimension4];
			}
		}
	}
	return array4D;
...
float bicubic_Bspline_calculation(int I, int J, float x2, float y2, float**** lookup_table) {
	float x_decimal = x2 - I;
	float y_decimal = y2 - J;

	float x_2_decimal = x_decimal * x_decimal;
	float y_2_decimal = y_decimal * y_decimal;

	float x_3_decimal = x_2_decimal * x_decimal;
	float y_3_decimal = y_2_decimal * y_decimal;

	float GrayIntensity = 0.f;
	float**& coefficient = lookup_table[J][I];

	GrayIntensity += coefficient[0][0];
	GrayIntensity += coefficient[0][1] * x_decimal;
	GrayIntensity += coefficient[0][2] * x_2_decimal; 
	GrayIntensity += coefficient[0][3] * x_3_decimal;

	GrayIntensity += coefficient[1][0] * y_decimal;
	GrayIntensity += coefficient[1][1] * y_decimal * x_decimal;
	GrayIntensity += coefficient[1][2] * y_decimal * x_2_decimal;
	GrayIntensity += coefficient[1][3] * y_decimal * x_3_decimal;

	GrayIntensity += coefficient[2][0] * y_2_decimal;
	GrayIntensity += coefficient[2][1] * y_2_decimal * x_decimal;
	GrayIntensity += coefficient[2][2] * y_2_decimal * x_2_decimal;
	GrayIntensity += coefficient[2][3] * y_2_decimal * x_3_decimal;

	GrayIntensity += coefficient[3][0] * y_3_decimal;
	GrayIntensity += coefficient[3][1] * y_3_decimal * x_decimal;
	GrayIntensity += coefficient[3][2] * y_3_decimal * x_2_decimal;
	GrayIntensity += coefficient[3][3] * y_3_decimal * x_3_decimal;
...
float gradientX(int I, int J, float x2, float y2, float**** lookup_table) {
	...
	GrayIntensity += coefficient[1][0];
	GrayIntensity += coefficient[1][1] * x_decimal;
	GrayIntensity += coefficient[1][2] * x_2_decimal;
	GrayIntensity += coefficient[1][3] * x_3_decimal;

	GrayIntensity += coefficient[2][0] * 2 * y_decimal;
	GrayIntensity += coefficient[2][1] * 2 * y_decimal * x_decimal;
	GrayIntensity += coefficient[2][2] * 2 * y_decimal * x_2_decimal;
	GrayIntensity += coefficient[2][3] * 2 * y_decimal * x_3_decimal;

	GrayIntensity += coefficient[3][0] * 3 * y_2_decimal;
	GrayIntensity += coefficient[3][1] * 3 * y_2_decimal * x_decimal;
	GrayIntensity += coefficient[3][2] * 3 * y_2_decimal * x_2_decimal;
	GrayIntensity += coefficient[3][3] * 3 * y_2_decimal * x_3_decimal;
...
float gradientY(int I, int J, float x2, float y2, float**** lookup_table) {
	...
	GrayIntensity += coefficient[0][1] * 1 * 1;
	GrayIntensity += coefficient[0][2] * 2 * x_decimal;
	GrayIntensity += coefficient[0][3] * 3 * x_2_decimal;

	GrayIntensity += coefficient[1][1] * 1 * y_decimal * 1;
	GrayIntensity += coefficient[1][2] * 2 * y_decimal * x_decimal;
	GrayIntensity += coefficient[1][3] * 3 * y_decimal * x_2_decimal;

	GrayIntensity += coefficient[2][1] * 1 * y_2_decimal * 1;
	GrayIntensity += coefficient[2][2] * 2 * y_2_decimal * x_decimal;
	GrayIntensity += coefficient[2][3] * 3 * y_2_decimal * x_2_decimal;

	GrayIntensity += coefficient[3][1] * 1 * y_3_decimal * 1;
	GrayIntensity += coefficient[3][2] * 2 * y_3_decimal * x_decimal;
	GrayIntensity += coefficient[3][3] * 3 * y_3_decimal * x_2_decimal;
